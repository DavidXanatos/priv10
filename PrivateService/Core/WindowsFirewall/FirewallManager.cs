...
namespace PrivateWin10
{
...
#endif
    {
        ...
        public void EvaluateRules(ProgramSet progSet, bool StrictTest = false)
        {
            ...
            /*else if ((MergedStat.AllowAll & (int)FirewallRule.Directions.Outboun) != 0)
                progSet.config.CurAccess = ProgramSet.Config.AccessLevels.OutBoundAccess;
            else if ((MergedStat.AllowAll & (int)FirewallRule.Directions.Inbound) != 0)
                progSet.config.CurAccess = ProgramSet.Config.AccessLevels.InBoundAccess;*/
        ...
        public void ApplyRules(ProgramSet progSet, UInt64 expiration = 0)
        {
            ...
            if (progSet.config.NetAccess == ProgramSet.Config.AccessLevels.Unconfigured)
                return;
            ...
            foreach (Program prog in progSet.Programs.Values)
            {
                ...
                for (int i = 1; i <= 2; i++)
                {
                    ...
                    /*if((progSet.config.NetAccess == ProgramSet.Config.AccessLevels.InBoundAccess && direction != FirewallRule.Directions.Inbound)
                     && (progSet.config.NetAccess == ProgramSet.Config.AccessLevels.OutBoundAccess && direction != FirewallRule.Directions.Outboun))
                        continue;*/
