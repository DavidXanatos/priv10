...
using static PrivateWin10.WindowsFirewall;
    ...
    public class WinNetFwAPI : IDisposable
    {
        ...
        public bool UpdateRule(FirewallRule rule)
        {
            ...
            if (uRet != ERROR_SUCCESS)
            {
                ...
                if(message != null)
                    App.LogError("Failed to Write rule, status-code: {0} ({1})", entry.Status.ToString(), message);
                else
                    App.LogError("Failed to Write rule, error-code: {0} ({1})", uRet.ToString(), new Win32Exception((int)uRet).Message);
            }
            else if (bAdd)
            {
                rule.Index = RuleCounter++;
